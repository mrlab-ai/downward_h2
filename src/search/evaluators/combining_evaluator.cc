...
EvaluationResultBatched CombiningEvaluator::compute_result(
    EvaluationContextBatched &eval_context) {
    // This marks no preferred operators.
    EvaluationResultBatched result;
    ...
    for (int i = 0; i < bs; i++) {
        ...
        for (const shared_ptr<Evaluator> &subevaluator : subevaluators) {
            int value = eval_context.get_evaluator_value_or_infinity(subevaluator.get())[i];
            if (value == EvaluationResult::INFTY) {
                evaluator_values.push_back(value);
                was_infinite = true;
                break;
        ...
        evaluator_values.push_back(combine_values(values));

    }
    result.set_evaluator_values(evaluator_values);
    // If we arrived here, all subevaluator values are finite. TODO: Let's hope if this is still true
    return result;
}
