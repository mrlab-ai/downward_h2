...
#else
    static const std::string proc_status_field = []() {
        const char *value = std::getenv("FD_MEMORY_FIELD");
        if (!value || !*value) {
            return std::string("VmHWM:");
        }
        std::string field(value);
        if (field.back() != ':') {
            field.push_back(':');
        }
        if (field == "VmPeak:" || field == "VmHWM:" || field == "VmRSS:") {
            return field;
        }
        return std::string("VmHWM:");
    }();
