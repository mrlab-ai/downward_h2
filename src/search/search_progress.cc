...
bool SearchProgress::check_progress(const EvaluationContextBatched &eval_context) {
    bool boost = false;
    int bs = eval_context.get_batch_size();
    for (int i = 0; i < bs; i++) {
        eval_context.get_cache().for_each_evaluator_result(i,
        [this, &boost](const Evaluator *eval, const EvaluationResultBatched &result, int idx) {
            if (eval->is_used_for_reporting_minima() || eval->is_used_for_boosting()) {
                if (process_evaluator_value(eval, result.get_evaluator_value(idx))) {
                    if (eval->is_used_for_reporting_minima()) {
                        eval->report_new_minimum_value(result, idx);
                    }
                    if (eval->is_used_for_boosting()) {
                        boost = true;
                    }
