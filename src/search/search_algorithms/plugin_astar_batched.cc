...
namespace plugin_astar_batched {
class AStarBatchedSearchFeature
    : public plugins::TypedFeature<SearchAlgorithm, eager_search_batched::EagerSearchBatched> {
public:
    AStarBatchedSearchFeature() : TypedFeature("astarbatched") {
        document_title("A* search (eager, batched)");
        document_synopsis(
        ...
        eager_search_batched::add_eager_search_options_to_feature(
            *this, "astarbatched");
    ...
    virtual shared_ptr<eager_search_batched::EagerSearchBatched> create_component(
        const plugins::Options &opts,
        const utils::Context &) const override {
        plugins::Options options_copy(opts);
        auto temp =
            search_common::create_astar_batched_open_list_factory_and_f_eval(
        ...
        return plugins::make_shared_from_arg_tuples<eager_search_batched::EagerSearchBatched>(
            ...
            eager_search_batched::get_eager_search_arguments_from_options(
                options_copy)
            );
    }
};

static plugins::FeaturePlugin<AStarBatchedSearchFeature> _plugin;
}
