...
// static void dump_atom(const FactPair &atom) {
//     std::cout << "h(" << atom.var << ", " << atom.value << "), ";
// }

// static void dump_tuple(std::vector<FactPair> t) {
//     for (auto &atom : t) {
//         dump_atom(atom);
//     }
// }
        ...
        for (OperatorProxy op : task_proxy.get_operators()) {
            ...
            if (c1 != numeric_limits<int>::max()) {
                ...
                for (Tuple &partial_eff : partial_effs) {
                    ...
                    //dump_atom(partial_eff[0]);
                    //std::cout << "\t\tc1 = " << c1 + op.get_cost() << std::endl;
        ...
        if (!contradict && (tuple.size() > t.size()) && (check_tuple_in_tuple(t, tuple) == 0)) {
            ...
            if (is_valid) {
                ...
                if (c2 != numeric_limits<int>::max()) {
                    ...
                    //dump_tuple(tuple); // Change the parameter type of the dump_tuple function to match the type of the tuple variable.
                    //std::cout << "\tc2 = " << c2 + op.get_cost() << std::endl;
                }
            }
        }
    }
...
void HMHeuristic::dump_table() const {
    for (auto &hm_ent : hm_table) {
        if (hm_ent.first.size() == 1) {
            std::cout << "h(" << hm_ent.first << ") = " << hm_ent.second << std::endl;
        }
    }
    for (auto &hm_ent : hm_table) {
        if (hm_ent.first.size() == 2) {
            std::cout << "h(" << hm_ent.first << ") = " << hm_ent.second << std::endl;
        }
    }
}
