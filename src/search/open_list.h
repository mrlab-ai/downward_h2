...
protected:
    ...
    */
    ...
    virtual void do_insertion(EvaluationContextBatched &eval_context,
                              const Entry &entry, size_t idx) {
      (void)eval_context;
      (void)entry;
      (void)idx;
      throw std::runtime_error("Not implemented"); //TODO: there is no open_list.cc -> can we design this better?
    };
...
public:
    ...
    */
    ...
    void insert(EvaluationContextBatched &eval_context, const Entry &entry, size_t idx);
    ...
    */
    ...
    virtual bool is_dead_end(EvaluationContextBatched &eval_context, int idx) const {
      (void)eval_context;
      (void)idx;
      throw std::runtime_error("Not implemented"); //TODO: there is no open_list.cc -> can we design this better?
    };
    virtual std::vector<bool> is_dead_end(EvaluationContextBatched &eval_context) const {
      (void)eval_context;
      throw std::runtime_error("Not implemented"); //TODO: there is no open_list.cc -> can we design this better?
    };
...
void OpenList<Entry>::insert(
    ...
    if (only_preferred && !eval_context.is_preferred())
        return;
    if (!is_dead_end(eval_context, idx)) {
        do_insertion(eval_context, entry, idx);
    }
